...
\label{sec:conducao}
    ...
    \subsection{Equação diferencial da condução de calor}
        ...
        Finalmente, também podemos citar aqui uma terceira categoria de condições de contorno, não citada até agora, chamada condições de contorno de Robin. Este tipo de condição de contorno envolve tanto a função incógnita ($T\mathbf{x}$) e sua derivada ($\frac{\partial T}{\partial \mathbf{n}}$). No caso de condução de calor, esta é usada para representar o fenômeno de convecção:

        \begin{equation}
            q = k\frac{\partial T}{\partial \mathbf{n}} = h\left(T(\mathbf{x}) - T_\infty\right) \quad, x \in \Gamma_3 \subset \partial \Omega
            \label{eq:conveccao}
        \end{equation}

        \noindent no qual $\Gamma_3$ é uma terceira superfície, sujeita à convecção. A variável $h$ é o coeficiente de convecção e $T_\infty$ é a temperatura do fluido externo.
    ...
    \subsection{Método de Galerkin}
        ...
        \subsubsection*{Aplicação da condição de contorno de convecção}

        As condições de contorno de Dirichlet e de Neumann são aplicadas, neste caso, de forma similar aos casos anteriores. A primeira entra ``cortando'' linhas e colunas da matriz de rigidez (e somando um termo no vetor de forças caso esta seja não nula); a segunda aparece no vetor de força. No problema de condução de calor, aparece uma terceira condição de contorno, a de Robin (Eq. \eqref{eq:conveccao}).

        Considere um elemento no qual não há geração de calor ou condições de Neumann para simplificar as expressões, sem perda de generalidade. Este é representado por:
        ...
        Podemos também escrever a convecção como:

        \begin{equation}
            q = \left(\mathbf{q} \cdot \mathbf{n}\right) = h\left(T(\mathbf{x}) - T_\infty\right) = h \left(\mathbf{N} \mathbf{a}_e - T_\infty\right)
        \end{equation}
        ...
        \begin{equation}
            \int_{\Omega_e} \mathbf{B}^T \mathbf{D} \mathbf{B} dA \; \mathbf{a}_e = 
            -\int_{\Gamma_e} \mathbf{N}^T h \left(\mathbf{N} \mathbf{a}_e - T_\infty\right) d\Gamma
        \end{equation}

        \begin{equation}
            \left[\int_{\Omega_e} \mathbf{B}^T \mathbf{D} \mathbf{B} dA +  \int_{\Gamma_e} \mathbf{N}^T  h \mathbf{N} d\Gamma_3\right] \; \mathbf{a}_e = 
            \int_{\Gamma_e} \mathbf{N}^T  h T_\infty  d\Gamma_3
        \end{equation}

        Portanto, a convecção aparece por meio de dois termos: um a direita, que compõe o vetor de forças, e um a esquerda, que compõe a matriz de rigidez.
