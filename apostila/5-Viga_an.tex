...
\section{Revisão modelo de viga}
    ...
    \subsection{Funções de singularidade}

        Para resolver esta equação diferencial, usamos novamente as funções de singularidade. Neste caso, no entanto, usamos também a função $\left\langle x-a\right\rangle^{-2}$, a derivada do delta de Dirac. Esta é utilizada para representar binários aplicados na viga. Atente-se que, no entanto, o momento representado por essa função de singularidade está no sentido oposto ao da regra da mão direita.
        
        A Figura \ref{fig:singularidade_viga} mostra algumas funções de singularidade, incluindo esta.
